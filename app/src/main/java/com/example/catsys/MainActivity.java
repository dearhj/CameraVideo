...
import com.example.catsys.method.SetPreviewAndCapture;
import com.example.catsys.method.VideoRecordUtils;
import com.example.catsys.util.ImageSaver;
import com.example.catsys.util.Utils;
import com.example.catsys.view.ResizeAbleSurfaceView;
...
public class MainActivity extends AppCompatActivity{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        picture.setOnClickListener(v -> {
            ...
            Utils.toast(this, "正在保存");
        });
        change.setOnClickListener(v -> switchCamera());
        record.setOnClickListener(v -> {
            videoRecordUtils.startRecord(MainActivity.this,handler);
            Utils.toast(this, "开始录像");
        });
        stopRecord.setOnClickListener(v -> {
            videoRecordUtils.stopRecord();
            setPreviewAndCapture.startPreview();
            videoRecordUtils.create(surfaceView, cameraDevice, VideoRecordUtils.WH_1920X1080);
            Utils.toast(this, "结束录像");
        });
        zoom.setOnClickListener(v -> setPreviewAndCapture.setZoom());
...
//            imageReader = ImageReader.newInstance(previewSize.getWidth(), previewSize.getHeight(), ImageFormat.JPEG,10);
            imageReader = ImageReader.newInstance(4224, 3136, ImageFormat.JPEG,10);
            //设置回调处理接受图片数据
